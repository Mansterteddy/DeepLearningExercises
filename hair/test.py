...
pretrained_network = ['inception_v3', 'resnet101']
resume = False
...
if pretrained_model:
    ...
    if resume:
        ...
        checkpoint = torch.load('./checkpoint/ckpt1.t7')
        net = checkpoint['net']
...
# Training
# Seems like we should convert outputs to outputs[0] in training mode, but don't change anything in test mode.
def train(epoch):
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/ckpt1.t7')
        best_acc = acc
